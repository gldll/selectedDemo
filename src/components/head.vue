        ...
        <div class="header-container clearfix">
            ...
            <div class="search-container fr">
                <input type="text"
                       ...
                       @input="getSuggest(search.value)">
                <div class="suggest">
                    <ul>
                        <li class="active"
                            v-for="value in search.suggest"
                            :key="value">{{value}}</li>
                    </ul>
                </div>
                <a href="javascript:;"
                   class="search-btn fl">搜索</a>
                <i class="icon"></i>
                <i class="placeholder"
                   v-if="search.placeholder && !search.value">{{search.placevalue}}</i>
                <div class="keyword-list clearfix">
                    <a href="javascript:;"
                       class="fl">运动装备 7.5折起</a>
                    <a href="javascript:;"
                       class="fl">蚕丝被 431元起</a>
                    <a href="javascript:;"
                       class="fl">补水 保湿</a>
                    <a href="javascript:;"
                       class="nbr fl">抗皱家居服</a>
                </div>
            </div>
        </div>
        <nav @mouseleave="setMenu (-1)">
            <ul class="catalog-href-list clearfix">
                ...
                <li class="catalog-href fl"
                    :key="value"
                    v-for="value,index in menu.menus"
                    @mouseenter="setMenu(index)">
                    <a href="javascript:;"
                       class="title">{{value}}</a>
                </li>
            ...
            <div class="catalog-list">
                <ul class="clearfix">
                    <li class="catalog-column fl"
                        v-for="value in menu.twoMenus[menu.index] "
                        :key="value.title">
...
export default {
    data () {
        return {
            search: {
                placevalue: "张一山同款 开衫体恤",
                value: "",
                placeholder: true,
                suggest: []
            },
            menu: {
                menus: ["居家生活", "服饰鞋包", "个户清理", "母婴亲子", "运动旅行", "数码家电", "礼品特色"],
                twoMenus: [],
                index: -1,
                timer: null
            }
    ...
    methods: {
        async getSuggest (value) {
            ...
            this.search.suggest = await this.$http.getSuggest(value)
        },
        setMenu (index) {
            clearTimeout(this.menu.timer)

            this.menu.timer = setTimeout(async () => {

                this.menu.index = index
                if (index != -1) {
                    this.menu.twoMenus[index] || (this.$set(this.menu.twoMenus, index, await this.$http.getMenu(index)))
...
.header .catalog-href.active a,
.header .catalog-href:hover a {
    color: #b4a078;
